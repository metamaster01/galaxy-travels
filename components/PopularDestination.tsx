...
import { motion } from "framer-motion";
import { getPopularTrips } from "@/data/trips";
...
export function PopularTrips({
  ...
  /** Pass the current trip's id on detail pages so it doesn't show up in its own related-trips row. */
  excludeId?: string;
  heading?: string;
  eyebrow?: string;
}) {
  const trips = getPopularTrips(excludeId);
  ...
  return (
    <section className="bg-night px-4 py-16 sm:px-6 sm:py-20">
      <div className="mx-auto max-w-6xl">
        ...
        {/* Horizontal scroll-snap strip: holds the whole popular set, not
            just the few visible at a time — drag or scroll for the rest. */}
        <div className="-mx-4 mt-10 flex snap-x snap-mandatory gap-4 overflow-x-auto px-4 pb-2 sm:mx-0 sm:gap-5 sm:px-0 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          {trips.map((trip) => (
